...
def get_data_for_respi_phase(sujet, cond):
...
def process_dfc_res(sujet, cond, export_type):
    ...
    if export_type == 'whole':
        ...
        #### load data 
        allband_data = get_data_for_respi_phase(sujet, cond)
...
def process_dfc_res_summary(sujet):
    ...
    for cond in prms['conditions']:
        #### load data
        allcond_data_i = get_data_for_respi_phase(sujet, cond)
        ...
        for mat_type_i, mat_type in enumerate(cf_metrics_list):
            ...
            for band in band_name_fc_dfc:
                ...
                for respi_phase in respi_phase_list:
                    ...
                    max_list = np.append(max_list, allcond_data_i[respi_phase][band][mat_type_i,:,:].max())
                    max_list = np.append(max_list, np.abs(allcond_data_i[respi_phase][band][mat_type_i,:,:].min()))
        ...
        for band in band_name_fc_dfc:
            ...
            allcond_data_diff_i[band] = allcond_data_i['inspi'][band] - allcond_data_i['expi'][band]
...
if __name__ == '__main__':
    ...
    for sujet in sujet_list_FR_CV:
        ...
        for cond in prms['conditions']:
            ...
            for export_type in ['whole', 'respi_phase']:
