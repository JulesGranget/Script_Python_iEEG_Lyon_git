...
from n02bis_prep_adjust_values import *
...
def extract_data_trc(sujet):
    ...
    if sujet in ['VERj', 'DUCa', 'CARv', 'BOUt', 'FLAb', 'LAVs']:
    ...
    if debug:
        ...
        file_name = 'LYONNEURO_2025_BOUt_RESPI3.trc'
        ...
        if srate_whole_file != [srate_whole_file[i] for i in range(len(srate_whole_file))] :
            raise ValueError('srate different in segments')
        else :
        ...
        chan_plot = 'p15+'
        chan_i = chan_list_file.index(chan_plot)
        plt.plot(data_file[chan_i,:])
        plt.show()

        data_file.shape[-1]/srate_file
    ...
    if debug:
        ...
        srate_global_file = srate_whole[file_i]
    ...
    srate_global_file = srate_whole[0]
    ...
    if len(trc_file_names) > 1 :
        ...
        for trc_i, trc_name in enumerate(trc_file_names_ordered): 
            ...
            if trc_i == 0 :
                ...
                if srate_global_file != srate_whole[trc_i]:
    ...
    #### verify srate
    if srate_global_file != srate:

        print(f'######## RESAMPLE NEEDED {sujet} ########')

        time_vec = np.linspace(0,1,data.shape[-1])
        time_vec_resample = np.linspace(0,1,int(data.shape[-1] * (srate / srate_global_file)))

        data_resampled = np.zeros((data.shape[0], time_vec_resample.shape[0]))

        for chan_i in range(data.shape[0]):
            f = scipy.interpolate.interp1d(time_vec, data[chan_i,:], kind='linear', fill_value="extrapolate")
...
def extract_data_trc_bi(sujet):
    ...
    if debug:
        ...
        file_name = 'LYONNEURO_2025_FLAb_FR_CV.trc'
        reader = neo.MicromedIO(filename=file_name)
        seg = reader.read_segment()
        print('len seg : ' + str(len(seg.analogsignals)))
        ...
        chan_plot = aux_chan.get(sujet).get('nasal')
        chan_i = chan_list_file.index(chan_plot)
        plt.plot(data_file[chan_i,:])
        plt.show()

        chan_plot = aux_chan.get(sujet).get('ECG')
        chan_plot = 'PULS+'
        for chan_plot in ['p16+', 'p17+', 'emg1', 'emg2', 'MKR3+', 'p19+', 'p20+', 'p21+', 'p22+', 'p23+', 'p24+', 'MKR4+', 'PULS+', 'BEAT+', 'SpO2+']:
            chan_i = chan_list_file.index(chan_plot)
            plt.plot(data_file[chan_i,:])
            plt.title(chan_plot)
            plt.show()
...
def organize_raw(sujet, raw):
    ...
    srate_global_file = int(raw.info['sfreq'])
    ...
    for sujet in sujet_list_FR_CV:
        ...
        for monopol in [True, False]:
            ...
            if debug:
                ...
                trigger_allsujet[sujet]['trig_time']
                trig_pat_VS = [531000, 684600]

                chan_name = 'nasal'
                chan_i = chan_list_aux.index(chan_name)
                plt.plot(data_aux[chan_i,:])
                plt.vlines(trig_pat_VS, ymin=data_aux[chan_i,:].min(), ymax=data_aux[chan_i,:].max(), colors='k')
                plt.show()
            ...
            if monopol:
                prepstep_sel = prep_step_wb_monopol
            else:
                prepstep_sel = prep_step_wb_bipol

            data_preproc_wb  = preprocessing_ieeg(data, chan_list, srate, prepstep_sel)
