...
def compute_stretch_tf_dB(sujet, tf, cond, session_i, respfeatures_allcond, stretch_point_TF, band, band_prep, nfrex, srate, monopol):
    ...
    #### verify cycle number
    n_cycles_stretch_sam = stretch_data_tf(respfeatures_allcond[cond][session_i], stretch_point_TF, tf[0,:,:], srate)[0].shape[0]
        ...
        for band, freq in freq_band.items():
            ...
            for cond in cond_to_compute:
                ...
                for session_i in range(n_session):
                    data = load_data_sujet(sujet, band_prep, cond, session_i, monopol)[:len(chan_list_ieeg),:]
                    n_cycle_list.append(stretch_data_tf(respfeatures_allcond[cond][session_i], stretch_point_TF, data[:,:], srate)[0].shape[0])
