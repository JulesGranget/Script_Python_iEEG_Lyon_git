...
def sync_folders__push_to_mnt():
    ...
    folder_to_push_to = {path_data : os.path.join(path_mntdata, 'Data'), path_precompute : os.path.join(path_mntdata, 'Analyses', 'precompute'), 
                         path_prep : os.path.join(path_mntdata, 'Analyses', 'preprocessing'), path_slurm : os.path.join(path_mntdata, 'Script_slurm'), 
                         path_anatomy : os.path.join(path_mntdata, 'Analyses', 'anatomy'), 
                         path_main_workdir : os.path.join(path_mntdata, 'Script_Python_iEEG_Lyon_git'),
                         os.path.join(path_results, 'allplot', 'df') : os.path.join(path_mntdata, 'Analyses', 'results', 'allplot', 'df'),
                         os.path.join(path_results, 'allplot', 'RESPI') : os.path.join(path_mntdata, 'Analyses', 'results', 'allplot', 'RESPI')}
            
    #### We push from A to B
    #folder_local, folder_remote = path_anatomy, os.path.join(path_mntdata, 'Analyses', 'anatomy')
...
def write_script_slurm(name_script, name_function, params_one_script, n_core, mem):
    ...
    lines = [f'#! {python}']
    lines += ['import sys']
    lines += [f"sys.path.append('{os.path.join(path_mntdata, 'Script_Python_iEEG_Lyon_git')}')"]
...
def load_respfeatures(sujet):
    ...
    os.chdir(os.path.join(path_respfeatures, 'allplot', 'RESPI', 'respfeatures'))
...
def stretch_data_fc(resp_features, nb_point_by_cycle, data, srate):
