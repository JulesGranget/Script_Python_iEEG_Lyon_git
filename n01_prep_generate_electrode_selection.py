    ...
    if len(trc_file_names) > 1 :
        ...
        for trc_i in range(len(trc_file_names)-1): 
            ...
            if chan_list != chan_list_whole[trc_i]:
                raise ValueError('not the same chan list')
...
def generate_plot_loca(sujet, chan_list_trc):
    ...
    if sujet in ['VERj', 'DUCa', 'CARv', 'BOUt', 'FLAb', 'LAVs']:
        freesurfer_destrieux = [ parcellisation['MNI-Destrieux'][parcellisation['contact'] == nchan].values.tolist()[0] for nchan in plot_csv_extract]
    else:
