...
df_onepair <- subset(df_raw, phase == 'W')
...
for (band_sel in band_list) { 
  ...
  for (pair_sel in pair_list) {
    ...
    df <- df_onepair[c("sujet", "phase", "pair_i", "fc", "resp")]
    ...
    #### model
    complex_form <- fc ~ resp + (phase | sujet/pair_i)
    simple_form  <- fc ~ resp + (1 | sujet)
    simple_form_refit  <- fc ~ resp
...
for (band_sel in band_list) { 
  ...
  for (pair_sel in pair_list) {
    ...
    #df <- df_oneROI[c("sujet", "Cxy", "cond", "resp")]
    df <- df_onepair[c("sujet", "phase", "pair_i", "fc", "cond", "resp")]
