...
def load_Cxy(sujet, conditions, monopol):
    ...
    Cxy_allcond = {'Cxy' : {}, 'Cxy_surr' : {}}

    for cond in conditions:
            
        if monopol:

            Cxy_val = np.load(f'{sujet}_{cond}_Cxy.npy')[:len(chan_list_ieeg),:]
            Cxy_surrogates = np.load(f'{sujet}_{cond}_Cxy_mask.npy')[:len(chan_list_ieeg),:]

        else:
            
            Cxy_val = np.load(f'{sujet}_{cond}_Cxy_bi.npy')[:len(chan_list_ieeg),:]
            Cxy_surrogates = np.load(f'{sujet}_{cond}_Cxy_mask_bi.npy')[:len(chan_list_ieeg),:]

        #### median
        Cxy_allcond['Cxy'][cond] = Cxy_val
        Cxy_allcond['Cxy_surr'][cond] = Cxy_surrogates
...
def load_MVL(sujet, conditions, monopol):
    ...
    for cond in conditions:
        ...
        MVL_val = np.zeros((session_count[cond], len(chan_list_ieeg), stretch_point_surrogates_MVL_Cxy))

        for session_i in range(session_count[cond]):

            if monopol:

                MVL_val[session_i,:,:] = np.load(f'{sujet}_{cond}_{str(session_i+1)}_MVL_wb.npy')[:len(chan_list_ieeg),:]

            else:
                
                MVL_val[session_i,:,:] = np.load(f'{sujet}_{cond}_{str(session_i+1)}_MVL_wb_bi.npy')[:len(chan_list_ieeg),:]

        #### median
        MVL_allcond[cond] = np.median(MVL_val, axis=0)
...
def export_Cxy_in_df(sujet, monopol):
    ...
    if monopol:
        ...
        if os.path.exists(os.path.join(path_results, 'allplot', 'df', 'subject_wise', f'{sujet}_df_Cxy.xlsx')):
            print('Cxy : ALREADY COMPUTED', flush=True)
            return

    else:

        if os.path.exists(os.path.join(path_results, 'allplot', 'df', 'subject_wise', f'{sujet}_df_Cxy_bi.xlsx')):
            print('Cxy : ALREADY COMPUTED', flush=True)
            return
    ...
    nwind, nfft, noverlap, hannw = get_params_spectral_analysis(srate)
    Cxy_allcond = load_Cxy(sujet, conditions, monopol)
    ...
    for cond in conditions:
        ...
        for chan_i, chan_name in enumerate(chan_list):
            ...
            Cxy_i = np.median(Cxy_allcond['Cxy'][cond][chan_i,hzCxy_sel])
            Cxy_surr_i = np.median(Cxy_allcond['Cxy_surr'][cond][chan_i,hzCxy_sel])

            data_export_i =   {'sujet' : [sujet], 'cond' : [cond], 'chan' : [chan_name], 'ROI' : [ROI_i], 'Lobe' : [Lobe_i], 'side' : [side_i], 
                            'Cxy' : [Cxy_i], 'Cxy_surr' : [Cxy_surr_i], 'resp' : [respi_median]}
            
            df_export_i = pd.DataFrame(data_export_i)
            
            df_export_Cxy = pd.concat([df_export_Cxy, df_export_i])

    #### save
    os.chdir(os.path.join(path_results, 'allplot', 'df', 'subject_wise'))

    if monopol:
        df_export_Cxy.to_excel(f'{sujet}_df_Cxy.xlsx')
    else:
        df_export_Cxy.to_excel(f'{sujet}_df_Cxy_bi.xlsx')
...
def export_Pxx_in_df(sujet, monopol):
    ...
    if monopol:
        ...
        if os.path.exists(os.path.join(path_results, 'allplot', 'df', 'subject_wise', f'{sujet}_df_Pxx.xlsx')):
            print('Pxx : ALREADY COMPUTED', flush=True)
            return

    else:

        if os.path.exists(os.path.join(path_results, 'allplot', 'df', 'subject_wise', f'{sujet}_df_Pxx_bi.xlsx')):
            print('Pxx : ALREADY COMPUTED', flush=True)
            return
    ...
    for cond in conditions:
        ...
        for chan_i, chan_name in enumerate(chan_list):
            ...
            for band, freq in freq_band_dict_FC_lmm['wb'].items():
                ...
                Pxx_whole = np.median(tf_chan[frex_sel,:])
                Pxx_inspi = np.median(tf_chan[frex_sel,:int(stretch_point_TF/2)])
                Pxx_expi = np.median(tf_chan[frex_sel,int(stretch_point_TF/2):])
                ...
                data_export_i =   {'sujet' : [sujet]*len(phase_list), 'cond' : [cond]*len(phase_list), 'chan' : [chan_name]*len(phase_list), 
                                    ...
                                    'resp' : [respi_median]*len(phase_list)}
...
def compilation_export_df(sujet, monopol):
    ...
    # #### export
    print(f'COMPUTE Pxx {sujet} {monopol}', flush=True)
    export_Pxx_in_df(sujet, monopol)

    print(f'COMPUTE Cxy {sujet} {monopol}', flush=True)
...
if __name__ == '__main__':
    ...
    execute_function_in_slurm_bash('n09_precompute_extract_df', 'export_Pxx_in_df', list_params)
