...
def save_fig_dfc(cond, mat, ROI_list):
    ...
    for mat_type in ['ispc', 'wpli']:
        ...
        for c, band in enumerate(band_name_dfc):
            ax = axs[c]
            ax.set_title(band)
            # ax.matshow(allband_data['reducedpairs'][band].loc[mat_type,:,:], vmin=scales['vmin'], vmax=scales['vmax'])
        ...
        for c, band in enumerate(band_name_dfc):
            ...
            #                                 vmin=scales['vmin'], vmax=scales['vmax'], colormap=mat_type_color[mat_type], facecolor='w', 
            #                                 textcolor='k')
    ...
    for mat_type in ['ispc', 'wpli']:
        ...
        for c, band in enumerate(band_name_dfc):
            ax = axs[c]
            ax.set_title(band)
            # ax.matshow(mat_dfc_clean[band].loc[mat_type,:,:], vmin=scales['vmin'], vmax=scales['vmax'])
        ...
        for c, band in enumerate(band_name_dfc):
            ...
            #                                 title=band, show=False, padding=7, fig=fig, subplot=(nrows, ncols, c+1),
            #                                 vmin=scales['vmin'], vmax=scales['vmax'], colormap=cm.seismic, facecolor='w', 
                                            # textcolor='k')
            mne.viz.plot_connectivity_circle(mat_dfc_clean[mat_type][band], node_names=roi_names, n_lines=None, 
                                            title=band, show=False, padding=7, fig=fig, subplot=(nrows, ncols, c+1),
                                            colormap=mat_type_color[mat_type], facecolor='w', 
